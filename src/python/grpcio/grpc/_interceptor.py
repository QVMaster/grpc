...
class _ServerInterceptorPipeline(object):

    def _continuation(self, index):
        return lambda context: self._run_interceptor_at_index(context, index)

    def __init__(self, interceptors, handler_factory):
        self.interceptors = interceptors
        self.handler_factory = handler_factory
            
    def _run_interceptor_at_index(self, context, index):
        if index == len(self.interceptors):
            return self.handler_factory(context)
        interceptor = self.interceptors[index]
        continuation = self._continuation(index+1)
        return interceptor.intercept_handler(context, continuation)

    def execute(self, rpc_event):
        return self._run_interceptor_at_index(rpc_event, 0)


def intercept_service_handler(interceptors, handler_factory, rpc_event):
    pipeline = _ServerInterceptorPipeline(interceptors, handler_factory)
    return pipeline.execute(rpc_event)